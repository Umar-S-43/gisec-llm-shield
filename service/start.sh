...
NUM_PARALLEL="${NUM_PARALLEL:=4}"      # -np/--parallel: number of server slots — see
                                        # docs/MANUAL_CONFIG.md (must match shield/main.py's
                                        # total_llama_slots)
...
# looks like — a flat total (however large) is not sufficient on its own. Default
# per-slot budget is sized to fit Profile D's ~2112-token maximum-cost requests
