...
\begin{itemize}
    ...
    \item \emph{Resolved during report preparation:} \texttt{service/start.sh}'s
    two launch bugs (\texttt{--host 0.0.0.0} never passed, and the invalid
    long-form \texttt{--np} flag) were fixed and merged as this report was
    being finalized. The same merge also identified that \texttt{-c}/
    \texttt{--ctx-size} is \texttt{llama-server}'s \emph{total} context,
    divided evenly across \texttt{-np} slots -- confirmed empirically
    (\texttt{--parallel 4 --ctx-size 2048} yields 512 tokens per slot, not
    2048) -- meaning the per-slot context available during this project's
    live sessions was smaller than the flat configured value implied. This
    does \emph{not} affect any result reported here: every recorded Profile D
    attack request was rejected by the Shield before being forwarded to
    \texttt{llama-server} at all (Section~\ref{sec:profiled}), and every
    legitimate probe request is far too small for context sizing to matter.
    It would matter for a future test that admits and forwards a
    large-but-under-budget request.
