...
| `FORWARD_TIMEOUT_SECONDS` | 120 | Shield's HTTP client timeout when forwarding to llama-server; must be `>=` the slowest client-side timeout any loadgen profile uses |
